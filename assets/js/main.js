...
data.forEach((elt) => {
  i++
  content.innerHTML += `<section id="sc${i}"></section>`
  let sc = document.getElementById(`sc${i}`)
  sc.innerHTML += `<img src="${elt.url}">`
  sc.innerHTML += `<p>${elt.question}</p>`
  elt.choice.forEach((elt2) => {
    if (elt2 == elt.answer) {
      sc.innerHTML += `<input type="radio" name="quiz${i}" id="${elt2}${i}" class="ans">`
      sc.innerHTML += `<label for="${elt2}${i}">${elt2}</label>`
    } else {
      sc.innerHTML += `<input type="radio" name="quiz${i}" id="${elt2}${i}">`
      sc.innerHTML += `<label for="${elt2}${i}">${elt2}</label>`
    }
  })
})